...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 max-w-3xl">
    ...
    <form action="{{ route('admin.questions.update', $question) }}" method="POST" class="p-6" id="questionForm">
        @csrf
        @method('PUT')
        
        <div class="mb-8">
            <textarea name="question_text" rows="3" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 p-4 text-gray-700 bg-gray-50 focus:bg-white" required>{{ old('question_text', $question->question_text) }}</textarea>
        ...
        <div class="mb-6">
            <div class="flex justify-between items-center mb-4">
                ...
                <button type="button" id="addAnswerBtn" class="text-sm text-blue-600 hover:text-blue-800 font-medium"><i class="fas fa-plus mr-1"></i> Thêm đáp án</button>
            </div>
        ...
        <div class="flex justify-end gap-3 mt-8 pt-6 border-t border-gray-100">
            ...
            <button type="submit" class="px-6 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-colors shadow-md flex items-center"><i class="fas fa-save mr-2"></i> Cập Nhật Lại</button>
        </div>
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const dbAnswers = @json($question->answers);
    ...
    if (Object.keys(oldAnswers).length > 0) {
        ...
        dbAnswers.forEach(ans => {
            // is_correct comes as 1/0 or true/false
            createAnswerRow(ans.answer_text, ans.is_correct == 1 || ans.is_correct === true);
        });
    }
